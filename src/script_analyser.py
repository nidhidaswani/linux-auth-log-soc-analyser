...
def parse_args():
    p = argparse.ArgumentParser(description="Analyse Linux auth.log and reconstruct SSH attack behaviour.")
    p.add_argument("-i", "--input", default="auth.log", help="Path to auth.log file")
    p.add_argument("--top", type=int, default=5, help="Top N results to display")
    p.add_argument("--json-out", default=None, help="Write full results to JSON file (for tooling integration)")
    p.add_argument("--timeline-limit", type=int, default=30, help="Number of timeline events to print")
    return p.parse_args()
...
def main():
    ...
    for ip, c in overallAtt.most_common(args.top):
        print(f"  {ip}: {c}")

    print("\nBreakdown by signal:")
    for name, counter in countsByAttType.items():
        print(f"\n{name}:")
        for ip, c in counter.most_common(args.top):
            print(f"  {ip}: {c}")
    ...
    for actor, c in overallSusp.most_common(args.top):
        print(f"  {actor}: {c}")

    print("\nBreakdown by suspicious behaviour type:")
    for name, counter in countsBySuspType.items():
        print(f"\n{name}:")
        for actor, c in counter.most_common(args.top):
            print(f"  {actor}: {c}")
    ...
    for t in timeline[:args.timeline_limit]:
    ...
    for ip, stats in sorted(ip_activity.items(), key=lambda x: (x[1]["failed"] * 3) + (x[1]["success"] * 6) + (x[1]["sudo"] * 6) + (x[1]["su"] * 5) + (x[1]["root_session"] * 8) + (x[1]["suspicious"] * 6), reverse=True):
    ...
    if args.json_out:
        output = {
            "input_file": logs,
            "top_attack_ips": overallAtt.most_common(args.top),
            "attack_breakdown": {name: counter.most_common(args.top) for name, counter in countsByAttType.items()},
